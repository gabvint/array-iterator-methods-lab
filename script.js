...
const nameFormat = people.map((person) => {
    
    person = person.replace(/,/g, "").split(' ').reverse().join(' ')
    return firstLast.push(person)
...
const adult = devs.some((person) => {
    return isAdultPresent = dateToday.getFullYear() - person.year >= 18
});
...
const nineteenOrOlder = devs.every ((person) => {

    return isEveryone19OrOlder = dateToday.getFullYear() - person >= 19
...
travelMethodCounts = travelMethods.reduce((acc, methods) => {
